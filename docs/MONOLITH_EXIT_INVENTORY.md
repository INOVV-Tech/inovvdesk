...
| Projects | `includes/modules/projects/project-schema.php`, `includes/modules/projects/project-permissions.php`, `includes/modules/projects/project-boards.php`, `includes/modules/projects/project-lists.php`, `includes/modules/projects/project-cards.php`, `includes/components/project-board-surface.php`, `includes/components/project-card-composer.php`, `includes/components/project-card-detail.php`, `includes/api/project-handler.php`, `assets/js/project-board.js`, `assets/js/project-card-detail.js` | Trello-style project boards: schema bootstrap (incl. Fase 2 detail tables `project_card_comments`/`project_card_checklists`/`project_card_checklist_items`/`project_card_attachments` e governança `project_board_members` + colunas `project_cards.is_archived`/`archived_at`), staff permission model com acesso por quadro (membros; admin bypass), board/list/card query and write models, card detail modal (description, comments, checklists, attachments), server-rendered board filters (assignee/priority/due/search, Fase 2), board rendering com coluna própria de cards arquivados, template picker (blank/development), member strip (admin-only), modal composers, AJAX endpoints, and board browser behavior. Covered by `tests/project-*.php` incl. `tests/project-board-filters-contract-test.php` e `tests/project-governance-contract-test.php`. See `docs/PROJECTS_MODULE.md` for the phased roadmap. |
