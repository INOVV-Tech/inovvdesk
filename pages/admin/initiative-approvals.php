...
<div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true" aria-labelledby="approval-rule-modal-title" data-server-edit-modal>
    <div class="fd-card w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden shadow-2xl" style="height:90vh"><div class="flex flex-none items-center justify-between gap-3 p-5 border-b border-theme-light bg-theme-primary"><div><h2 id="approval-rule-modal-title" class="text-lg font-semibold text-theme-primary"><?php echo e(t('Edit approval rule')); ?></h2><p class="text-xs text-theme-muted mt-1"><?php echo e(t('Changes apply to future approval requests.')); ?></p></div><a class="p-2 text-theme-muted hover:text-theme-primary" href="<?php echo e($baseUrl); ?>" aria-label="<?php echo e(t('Close')); ?>"><?php echo get_icon('x', 'w-5 h-5'); ?></a></div><div class="min-h-0 flex-1 p-5 overflow-y-auto overscroll-contain" style="min-height:0"><?php $renderRuleForm($editingRule, $ref, true); ?></div></div>
</div>
